...
const Dashboard = () => {
  ...
  // State to store last update time
  const [lastUpdate, setLastUpdate] = useState("");

  // Function to update the last update time
  const updateLastUpdateTime = () => {
    const current = new Date();
    const formattedTime = `${current.getDate().toString().padStart(2, "0")}/${
      (current.getMonth() + 1).toString().padStart(2, "0")
    }/${current.getFullYear()} ${current
      .getHours()
      .toString()
  ...
  useEffect(() => {
    // Update the last update time when the component mounts
  ...
  return (
    <Box m="20px">
      ...
      <Box>
          ...
          sx={{ fontSize: "18px", fontWeight: "bold" }} // Increased font size and bold
          >
        ...
        <Box>
          <Button
            onClick={updateLastUpdateTime} // Update the time when the button is clicked
          >
          </Button>
        </Box>
